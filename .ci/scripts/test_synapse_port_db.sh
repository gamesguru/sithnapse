...
#
# Expects Synapse to have been already installed with `uv sync --extra postgres`.
...
uv run update_synapse_database --database-config .ci/postgres-config-unported.yaml --run-background-updates
